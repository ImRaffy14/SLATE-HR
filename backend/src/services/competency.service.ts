...
import { validateWeight, validateProficiencyLevels, validateRating, ProficiencyLevel } from '../utils/validation';
import { uploadFile, deleteFile } from './fileUpload.service';

export class CompetencyService {
  // Create competency with validations
  async createCompetencyService(data: {
    name: string;
    description?: string;
    categoryId: string;
    ...
    validateProficiencyLevels(data.levels);
    ...
    if (existingCompetency) {
      throw new AppError('Competency with this name already exists in this category', 400);
    }
  ...
  async getCompetenciesService(jobRoleId?: string) {
    // If jobRoleId is provided, first get categories that match the job role
    let categoryIds: string[] | undefined;
    if (jobRoleId) {
      const matchingCategories = await prisma.competencyCategory.findMany({
        where: {
          jobRoleIds: {
            has: jobRoleId
          }
  ...
  async getCompetenciesByEmployeePosition(employeeId: string) {
  ...
  async getCompetenciesByJobRoleId(jobRoleId: string) {
  ...
  // Update competency
  async updateCompetencyService(id: string, data: {
    name?: string;
    description?: string;
    categoryId?: string;
    levels?: ProficiencyLevel[];
  ...
  async deleteCompetencyService(id: string) {
    ...
    if (competency.employeeCompetencies.length > 0) {
      throw new AppError(
        `Cannot delete competency. It has ${competency.employeeCompetencies.length} employee assessment(s). Please remove them first.`,
        400
      );
    }
  ...
  private calculateFinalScore(managerRating: number | null, weight: number): number | null {
  ...
  async assignCompetencyToEmployee(data: {
    ...
    if (existing) {
      throw new AppError('Competency already assigned to this employee', 400);
    }
  ...
  async updateSelfRating(id: string, selfRating: number) {
    validateRating(selfRating, 'Self-rating');

    const employeeCompetency = await prisma.employeeCompetency.findUnique({
      where: { id },
      include: {
        competency: true
      }
    ...
    if (!employeeCompetency) {
      throw new AppError('Employee competency record not found', 404);
    }

    // Self-rating is stored but not used in final score calculation
    // Final score is calculated only from manager rating
    const finalScore = this.calculateFinalScore(
      employeeCompetency.managerRating,
      employeeCompetency.competency.weight
    );
  ...
  async updateManagerRating(id: string, managerRating: number, updatedBy: string, notes?: string) {
    validateRating(managerRating, 'Manager rating');
  ...
  async getSuggestedCompetenciesForEmployee(employeeId: string) {
    const employee = await prisma.employee.findUnique({
      where: { id: employeeId },
      include: {
        jobRole: true
    ...
    const assignedCompetencyIds = await prisma.employeeCompetency.findMany({
      where: {
        employeeId: employeeId
      },
      select: {
  ...
  async batchUpdateManagerRatings(data: {
    ...
    const employeeCompetencyIds = await prisma.employeeCompetency.findMany({
      where: {
        employeeId: data.employeeId,
        competencyId: {
    ...
    if (employeeCompetencyIds.length !== data.ratings.length) {
      throw new AppError('One or more competencies are not assigned to this employee', 400);
    }

    // Validate all ratings
    for (const rating of data.ratings) {
      validateRating(rating.rating, 'Manager rating');
    }

    // Update all ratings in a transaction
    const updated = await Promise.all(
      data.ratings.map(async (rating) => {
        const employeeCompetency = employeeCompetencyIds.find(
          ec => ec.competencyId === rating.competencyId
        );

        if (!employeeCompetency) {
          throw new AppError(`Competency ${rating.competencyId} not found for employee`, 404);
        }
    ...
    await Promise.all(
      data.ratings.map(rating =>
        this.runGapAnalysis(data.employeeId, rating.competencyId)
      )
    );
  ...
  // Run gap analysis
  async runGapAnalysis(employeeId: string, competencyId: string) {
    ...
    if (existingGap) {
      ...
      const updated = await prisma.gapAnalysis.update({
  ...
  // Get gap analysis
  async getGapAnalysis(employeeId: string, competencyId?: string) {
    const employee = await prisma.employee.findUnique({
      where: { id: employeeId }
    });
    ...
    if (competencyId) {
      ...
      if (gapAnalysis && gapAnalysis.recommendations && gapAnalysis.recommendations.length > 0) {
        const recommendationDetails = await prisma.trainingRecommendation.findMany({
          where: {
            id: {
    ...
    const gapAnalysesWithRecommendations = await Promise.all(
      gapAnalyses.map(async (gap) => {
  ...
  // Generate gap report
  async generateGapReport(employeeId: string) {
    const employee = await prisma.employee.findUnique({
      where: { id: employeeId },
      include: {
        competencies: {
    ...
    const gapAnalyses = await Promise.all(
      employee.competencies.map((ec: { competencyId: string }) => 
        this.runGapAnalysis(employeeId, ec.competencyId)
      )
    );
  ...
  private async linkRecommendationsToGap(gapAnalysisId: string, competencyId: string, currentLevel: number) {
