...
import { EmployeeMetrics, SnapshotData, PerformanceSummary } from '../types/performance.types';
...
export class AggregationService {
  ...
  async getEmployeeMetrics(employeeId: string): Promise<EmployeeMetrics> {
    ...
    const trainingsAttended = employee.trainingEnrollments.filter(
      te => te.attendance?.status === 'PRESENT' || te.attendance?.status === 'LATE'
    ).length;
   ...
   */
  async generatePerformanceSummary(employeeId: string): Promise<PerformanceSummary> {
    ...
    let competencyGrowth: 'High' | 'Moderate' | 'Low' | 'Stagnant' = 'Stagnant';
    if (metrics.avgCompetencyScore !== null) {
      if (metrics.avgCompetencyScore >= 80) competencyGrowth = 'High';
      else if (metrics.avgCompetencyScore >= 60) competencyGrowth = 'Moderate';
      else if (metrics.avgCompetencyScore >= 40) competencyGrowth = 'Low';
    }

    // Determine learning activity
    let learningActivity: 'High' | 'Moderate' | 'Low' | 'None' = 'None';
    if (metrics.courseCompletionRate >= 80) learningActivity = 'High';
    else if (metrics.courseCompletionRate >= 50) learningActivity = 'Moderate';
    else if (metrics.courseCompletionRate > 0) learningActivity = 'Low';

    // Determine training attendance
    let trainingAttendance: 'Excellent' | 'Good' | 'Moderate' | 'Poor' = 'Poor';
    if (metrics.trainingAttendanceRate >= 90) trainingAttendance = 'Excellent';
    else if (metrics.trainingAttendanceRate >= 70) trainingAttendance = 'Good';
    else if (metrics.trainingAttendanceRate >= 50) trainingAttendance = 'Moderate';
   ...
   */
  async createSnapshot(employeeId: string, period?: string): Promise<SnapshotData> {
    const metrics = await this.getEmployeeMetrics(employeeId);
    ...
    const existingSnapshot = await prisma.performanceSnapshot.findFirst({
   ...
   */
  async syncAllSnapshots(): Promise<{ synced: number; failed: number }> {
   ...
   */
  async getEmployeeHistory(employeeId: string, limit = 12) {
