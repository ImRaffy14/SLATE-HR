...
export class ExplainabilityService {
   ...
   */
  generateExplanation(summary: PerformanceSummary, analysis: AIAnalysisResult) {
    const explanations: string[] = [];
    const keyMetrics: Array<{ metric: string; value: string; impact: string }> = [];

    // Explain performance trend
    if (analysis.performanceTrend === 'Declining') {
      explanations.push(
        `Performance is showing a declining trend. This was determined by comparing the latest rating of ${summary.lastRating}/5 with the overall ${summary.trend.toLowerCase()} trajectory.`
      );
      keyMetrics.push({
      ...
      explanations.push(
        `Performance is trending positively, with a last rating of ${summary.lastRating}/5 and consistent ${summary.trend.toLowerCase()} momentum.`
      );
      keyMetrics.push({
        metric: 'Performance Trend',
        value: summary.trend,
    ...
    if (summary.competencyGrowth === 'Stagnant' || summary.competencyGrowth === 'Low') {
      explanations.push(
        `Competency development shows ${summary.competencyGrowth.toLowerCase()} growth, indicating a need for targeted skill development.`
      );
      keyMetrics.push({
    ...
    if (summary.learningActivity === 'None' || summary.learningActivity === 'Low') {
      explanations.push(
        `Learning activity is ${summary.learningActivity.toLowerCase()}, which may be contributing to competency stagnation. Encouraging course enrollment could help.`
      );
      keyMetrics.push({
      ...
      explanations.push(
        `Strong learning engagement is demonstrated through high course activity.`
      );
    }
    ...
    if (summary.trainingAttendance === 'Poor') {
      explanations.push(
        `Training attendance is poor, which reduces exposure to structured skill development opportunities.`
      );
      keyMetrics.push({
    ...
    if (summary.skillGaps > 0) {
      ...
      explanations.push(
        `${summary.skillGaps} skill gap(s) identified, representing ${gapSeverity} areas for improvement.`
      );
      keyMetrics.push({
        metric: 'Skill Gaps',
   ...
   */
  private explainRiskLevel(riskLevel: string, summary: PerformanceSummary): string {
   ...
   */
  private getConfidenceFactors(summary: PerformanceSummary): Array<{ factor: string; status: string }> {
    return [
      {
