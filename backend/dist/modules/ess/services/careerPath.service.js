...
class CareerPathService {
     ...
     */
    async getCareerPath(employeeId) {
        ...
        const allJobRoles = await prisma_1.default.jobRole.findMany({
            orderBy: {
                name: 'asc'
            }
        });
        ...
        const careerPaths = await Promise.all(allJobRoles.map(async (jobRole) => {
            // Skip current job role
            if (jobRole.id === currentJobRoleId) {
                return null;
            }
            const readiness = await this.calculateReadiness(employeeId, jobRole.id, employeeCompetencies);
            return {
     ...
     */
    async getCareerPathDetails(employeeId, targetRoleId) {
        // Get employee
        const employee = await prisma_1.default.employee.findUnique({
            where: { id: employeeId },
            include: {
        ...
        // Get target job role
        const targetJobRole = await prisma_1.default.jobRole.findUnique({
            where: { id: targetRoleId }
        });
        if (!targetJobRole) {
            throw new appError_1.AppError('Target job role not found', 404);
        }
        // Get employee competencies
        const employeeCompetencies = await this.competencyService.getEmployeeCompetencies(employeeId);
        // Calculate detailed readiness
        const readiness = await this.calculateReadiness(employeeId, targetRoleId, employeeCompetencies);
        ...
        const competencyComparison = requiredCompetencies.map(reqComp => {
            const employeeComp = employeeCompetencies.find(ec => ec.competencyId === reqComp.id);
     ...
     */
    async calculateReadiness(employeeId, jobRoleId, employeeCompetencies) {
        ...
        const readinessPercentage = Math.round((competenciesMet / requiredCompetencies.length) * 100);
        return {
            readinessPercentage,
            competenciesMet,
     ...
     */
    async getRequiredCompetenciesForRole(jobRoleId) {
