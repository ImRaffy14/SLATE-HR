...
class PerformanceAnalyticsService {
     ...
     */
    async getTeamAnalytics(managerId) {
        ...
        const employeeData = employees.map(emp => {
            ...
            const insight = emp.aiInsights[0]?.analysisResult;
            ...
            if (insight?.riskLevel) {
                ...
                if (snapshot.overallScore < 50)
                    riskLevel = 'high';
                else if (snapshot.overallScore < 70)
                    riskLevel = 'medium';
            }
            if (riskLevel === 'low')
    ...
    async getDashboardSummary() {
        const [orgAnalytics, recentInsights] = await Promise.all([
            ...
            prisma_1.default.aIInsight.findMany({
                where: {
                    analysisType: 'FULL',
                    expiresAt: { gt: new Date() }
                },
        ...
        for (const insight of recentInsights) {
            const result = insight.analysisResult;
            const risk = result?.riskLevel?.toLowerCase() || 'medium';
            if (risk === 'low')
                insightsByRisk.low++;
            else if (risk === 'medium')
                insightsByRisk.medium++;
            else
                insightsByRisk.high++;
            const trend = result?.performanceTrend?.toLowerCase() || 'stable';
            if (trend === 'improving')
                insightsByTrend.improving++;
            else if (trend === 'stable')
                insightsByTrend.stable++;
            else
                insightsByTrend.declining++;
        }
        return {
            ...orgAnalytics,
            aiInsightsSummary: {
            ...
            recentHighRiskEmployees: recentInsights
                .filter(i => i.analysisResult?.riskLevel === 'High')
                .slice(0, 5)
                .map(i => ({
                employeeId: i.employeeId,
                employeeName: i.employee.name,
                department: i.employee.department,
                insight: i.analysisResult?.insightSummary
            }))
        };
    }
}
