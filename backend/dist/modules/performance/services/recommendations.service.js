...
class RecommendationsService {
     ...
     */
    async getRecommendations(employeeId, forceRefresh = false) {
        const summary = await this.aggregationService.generatePerformanceSummary(employeeId);
        const aiRecommendations = await this.groqService.generateRecommendations(employeeId, summary, forceRefresh);
        // Map AI recommendations to actual courses and trainings
        const mappedRecommendations = await this.mapToResources(employeeId, aiRecommendations.recommendations || []);
        return {
            employeeId,
     ...
     */
    async mapToResources(employeeId, recommendations) {
        const mappedRecs = [];
        ...
        const relevantCourses = await prisma_1.default.course.findMany({
            where: {
                status: 'PUBLISHED',
        ...
        const relevantTrainings = await prisma_1.default.training.findMany({
            where: {
        ...
        const currentEnrollments = await prisma_1.default.enrollment.findMany({
            where: { employeeId },
            select: { courseId: true }
        });
        const enrolledCourseIds = new Set(currentEnrollments.map(e => e.courseId));
        const currentTrainingEnrollments = await prisma_1.default.trainingEnrollment.findMany({
            where: { employeeId },
        ...
        for (const rec of recommendations) {
            ...
            if (rec.type === 'course' || rec.title?.toLowerCase().includes('course')) {
                ...
                const matchedCourse = relevantCourses.find(c => !enrolledCourseIds.has(c.id) &&
            ...
            if (rec.type === 'training' || rec.title?.toLowerCase().includes('training')) {
                ...
                const matchedTraining = relevantTrainings.find(t => !enrolledTrainingIds.has(t.id) &&
        ...
        if (mappedRecs.length < 3 && gaps.length > 0) {
            for (const gap of gaps) {
                if (mappedRecs.length >= 5)
                    break;
                const courseForGap = relevantCourses.find(c => c.taggedCompetencies.includes(gap.competencyId) &&
        ...
        mappedRecs.sort((a, b) => (priorityOrder[a.priority] || 3) - (priorityOrder[b.priority] || 3));
        return mappedRecs;
    }
    /**
     * Get quick recommendations without AI (based on gap analysis only)
     */
    async getQuickRecommendations(employeeId) {
        const gaps = await prisma_1.default.gapAnalysis.findMany({
            where: { employeeId },
            include: {
                competency: {
        ...
        for (const gap of gaps.slice(0, 5)) {
            ...
            const training = await prisma_1.default.training.findFirst({
                where: {
                    status: { in: ['OPEN', 'ONGOING'] },
                    taggedCompetencies: { has: gap.competencyId }
                }
